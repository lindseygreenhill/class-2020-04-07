...
ggplot(x, aes(x = earnings, fill = as.factor(tier))) +
  geom_histogram()
...
x %>% summarize(cor = cor(faculty, earnings))
x %>% summarize(cor = cor(sat, earnings))

x %>%
  select(sat, faculty, earnings) %>%
  cor()
...
x %>%
  ...
  labs(title = "Earnings by Faculty Salary") +
  theme_classic()
x %>%
  ggplot(aes(x = sat, y = earnings)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Earnings by SAT score") +
  theme_classic()
...
```{r  4, echo=FALSE}
summary(lm(formula = earnings ~ sat + faculty, data = x))

```
Intercept: the expected average earnings of a student where faculty salary and sat score
SAT estimate: faculty held constant, once can expect that for every increase in SAT score earnings will increase dby 24.5
Faculty estimate: same as above, but for .39
